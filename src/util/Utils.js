...
export default {
  util: {
    ...
    formatFloat(value) {
      if (value){
        return (isNaN(value.toString().replace(/[^\d.,-]/g, '').replace(/\./gi, "").replace(/\,/gi, "."))? 0 : Number(value.toString().replace(/[^\d.,-]/g, '').replace(/\./gi, "").replace(/\,/gi, ".")))
      } else {
        return 0
      }
    },
    ...
    getQtdCombinacoes(qtNumeros, qtDezenas) {
        if (!isNaN(qtNumeros)  && !isNaN(qtDezenas) && qtNumeros > 0 && qtDezenas > 0){
            var div = (this.fatorial(qtDezenas) * this.fatorial(qtNumeros-qtDezenas));
            if (div === 0){
                div = 1;
            }
